...
HEARTBEAT_INTERVAL = 2  # Intervalo de envío de heartbeats en segundos
LOG_FILE = "raft_log.json"

# Crear la red gráfica
graph = nx.DiGraph()
graph.add_nodes_from(NODES)
pos = nx.circular_layout(graph)  # Asignar posiciones antes de limpiar la gráfica
...
def elect_leader(candidates):
    ...
    sorted_candidates = sorted(timeouts, key=timeouts.get)
    votes = {node: 1 for node in candidates}  # Cada nodo se vota a sí mismo
    voted_nodes = set()
    messages = []
